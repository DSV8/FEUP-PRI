...
RESULTS_FILE="retrieval_config/results/results_sys1_trec.txt"
TREC_EVAL_RESULTS="retrieval_config/results/trec_eval_results_sys1.txt"
...
# Process each sys1 query file
for query_file in "$QUERY_DIR"/*sys1.json; do
    # Check if any matching files were found
    if [[ ! -e "$query_file" ]]; then
        echo "Error: No sys1 query files found in $QUERY_DIR"
        exit 1
    fi

    # Extract the query identifier (e.g., "1" from "query1_sys1.json")
    base_name=$(basename "$query_file" "_sys1.json")
    ...
    if ! ./scripts/retrieval_eval/query_solr.py --query "$query_file" --uri http://localhost:8983/solr --collection tech_videos | \
        ./scripts/retrieval_eval/solr2trec.py "$query_id" --run-id "sys1" >> "$RESULTS_FILE"; then
...
if ! cat "$RESULTS_FILE" | ./scripts/retrieval_eval/plot_pr.py --qrels "$QRELS_TREC_FILE" --output "$IMAGES_DIR/prec_rec_sys1.png"; then
...
echo "  - Plot: $IMAGES_DIR/prec_rec_sys1.png"
